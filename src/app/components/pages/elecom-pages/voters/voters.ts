...
export class Voters implements OnInit {
  ...
  // FIX 1: removed '4th'
  voterYears = ['1st', '2nd', '3rd'];
  showPassword = false;
  showConfirmPassword = false;
  photoPreview: string | null = null;

  hasDraft = false;
  statusFilter: 'all' | 'voted' | 'pending' = 'all';
  ...
  private restoreDraft(): void {
    ...
    try {
      ...
      this.newVoter = {
        ...
        course: draft.course ?? '',
        year: draft.year ?? '1st',
        // FIX 2: restore photo so submit can send it
        photo: draft.photoPreview ?? '',
      };
  ...
  closeModal(): void {
    ...
    const hasInput =
      this.newVoter.studentId || this.newVoter.name || this.newVoter.course || this.newVoter.photo;
    if (hasInput) this.saveDraft();
  ...
  get filteredVoters(): Voter[] {
    ...
    if (this.statusFilter === 'voted') list = list.filter((v) => v.hasVoted);
    if (this.statusFilter === 'pending') list = list.filter((v) => !v.hasVoted);
    if (!q) return list;
    return list.filter(
      (v) =>
  ...
  setFilter(filter: 'all' | 'voted' | 'pending'): void {
  ...
  get notVotedCount(): number {
    return this.totalVoters - this.votedCount;
  }
  get participationRate(): number {
    return this.totalVoters > 0 ? Math.round((this.votedCount / this.totalVoters) * 100) : 0;
  }
  ...
  onPhotoSelected(event: Event): void {
    ...
    if (file.size > 2 * 1024 * 1024) {
      Swal.fire('File too large', 'Photo must be under 2MB.', 'warning');
      return;
    }
  ...
  markVoted(voter: Voter): void {
    const updated: Voter = {
      ...voter,
      hasVoted: true,
      verifiedAt: new Date().toLocaleTimeString('en-PH', {
        hour: '2-digit',
        minute: '2-digit',
      }),
    };
  ...
  deleteVoter(voter: Voter): void {
    // FIX 3: backtick template literal restored
    Swal.fire({
      title: 'Delete voter?',
      ...
      if (r.isConfirmed) this.svc.deleteVoter(voter.id).subscribe(() => this.loadVoters());
    });
  }
  ...
  async addVoterSubmit(): Promise<void> {
    ...
    if (!password) {
      Swal.fire('No password', 'Please set a login password.', 'warning');
      return;
    }
    ...
    try {
      // FIX 4: photo now passed to registerVoter so it saves and shows in list
      await this.authService.registerVoter({
        studentId,
        name,
        course,
        year,
      ...
      // FIX 3 (also here): backtick template literal restored
      const msg =
